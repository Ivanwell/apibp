...
const { devideOnChunks } = require('../../../scripts/common_functions/functions.js')
var fs = require('fs');
...
const GetProduct = async function (req, res, next) {
    ...
    if (brand) {
        filter = {article : req.query.article, brand : { $regex: req.query.brand, "$options": "i"  }}
    }
    
    try{
        
        const product = await Item_whole.findOne(filter).populate('link').populate({
            path: 'supliers',
            match: { amount: { $ne: '0' } },
        ...
        let dataPage = {
           ...
           //   description: product.unicTitle || product.title,
              ...
              'offers': {
                ...
                'price': product.supliers[0]?.price.toString() || 1,
                'priceCurrency': 'UAH',
        ...
        res.status(200).json({product : product, broadList: broadList, dataPage : dataPage});
...
const GetProductsByFullInfo = async function (req, res, next) {
    try {
        ...
        const products = await getProductsByModelAndEngine(category, brand, model,engine)
        
        const categoryNameSplited = category.split('/')
        const categoryName = categoryNameSplited[categoryNameSplited.length-1].toLowerCase()
        ...
        const articlesArr = products.map(item => item.article.replace(/[- ./]/g, '').toUpperCase())
        
        const articlesBrands = products.map(item => item.brand.toUpperCase())
        
        const productsWhole = await Item_whole.find({article : articlesArr, brand : articlesBrands}).limit(20).populate({
            path: 'supliers',
            options: { sort: { 'price': 1} },
            match: { amount: { $ne: '0' } },
            perDocumentLimit : 1,
        }).populate('categories').populate('link').exec();
        const productsCount = await Item_whole.find({article : articlesArr, brand : articlesBrands}).countDocuments();
...
const CheckCompatibility = async function (req, res, next) {
    try {
        ...
        const isThereSuitable = products.find(item => item.article.replace(/[- ./]/g, '').toUpperCase() === partArticle && item.brand.toUpperCase() === partBrand)
        
        if (isThereSuitable) {
            
            fs.appendFile('./routes/api/info/test.txt', `Підходить ${partArticle} ${partBrand} до ${model}`+ "\n", err => {
              if (err) {
                console.error(err);
              }
            })
            ...
            fs.appendFile('./routes/api/info/test.txt', `Не підходить ${partArticle} ${partBrand} до ${model}`+ "\n", err => {
              if (err) {
                console.error(err);
              }
            })
...
const GetProductsByCategory = async function (req, res, next) {
    ...
    try{
        ...
        const productsWhole = await Item_whole.find({categoryName : category}).skip(step).limit(20).populate({
            path: 'supliers',
            match: { amount: { $ne: '0' } },
            options: { sort: { 'price': 1} },
            perDocumentLimit : 1,
        ...
        const productsCount = await Item_whole.find({categoryName : category}).countDocuments();
        
    const obj = {
        arr : productsWhole,
        amount : productsCount,
        title : `${category}${page !=0 ? `, сторінка ${+page+1}` : ''}`,
        metaTitle : `Купити ${category.toLowerCase()} до Вашого авто${page !=0 ? `, сторінка ${+page+1}` : ''} - BAYRAKPARTS`,
...
const GetCategories = async function (req, res, next) {
    ...
    try {
        ...
        const category = await Category.findOne({link : link}).populate('categories').exec();
...
const GetDetailsByUuid = async function (req, res, next) {
    ...
    try{
        ...
        res.status(200).json({fits: detailsArr.product?.cars,
...
async function updatingFields (item) {
    ...
    const updating = await Item_whole.updateOne({_id : item.id}, {title : newTitle})}
}
...
const Test = async function (req, res, next) {
    ...
    try{
        ...
        const dataArr = await Item_whole.find({title : { $regex: 'Tiguan', $options: 'i' }})
...
module.exports = {GetProduct, GetProductsByFullInfo, GetProductsByCategory, GetCategories, CheckCompatibility, GetDetailsByUuid, Test, GetProducts}
